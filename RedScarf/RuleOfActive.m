...
-(void)show
{
    ...
    label.text = @"Q:薪资是按照什么规则计算的？\n A:y＝x＋0.25a＋0.25b，其中x为底薪，a为任务数，b为地址数\n\n举个例子，某学校的某兼职同学当天将3个任务送达到2个宿舍，底薪为5元。则当天的工资为：\ny＝5＋0.25＊3+0.25*2=6.25元";
...
-(void)initRule
{
    ...
    for (int i = 0; i < 5; i++) {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(50, i*60+26, kUIScreenWidth-80, 60)];
        label.textColor = MakeColor(75, 75, 75);
        label.numberOfLines = 0;
        label.text = textArray[i];
        label.font = [UIFont systemFontOfSize:14];
        [view addSubview:label];
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(15, i*58+43+i, 20, 20)];
