...
+(instancetype)shareViewWithPresentedViewController:(UIViewController *)controller items:(NSArray *)items title:(NSString *)title image:(UIImage *)image urlResource:(NSString *)url{
    XHCustomShareView *view = [[XHCustomShareView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    view.title = title;
    view.url = url;
    view.image = image;
    view.items = items;
...
-(void)itemClick:(UIButton *)btn{
    ...
    [[UMSocialDataService defaultDataService]  postSNSWithTypes:@[_items[index]] content:nil image:_image location:nil urlResource:nil presentedController:_controller completion:^(UMSocialResponseEntity *response){
